...
function getKey() {
  ...
  // Generate, persist, and log on first run
  _key = randomBytes(32);
  const hex = _key.toString('hex');
  writeFileSync(KEY_FILE, hex, 'utf8');
  console.warn(`[crypto] Generated encryption key. Set QW_ENCRYPTION_KEY=${hex} to override, or keep .qw_key safe.`);
  return _key;
}

export function isEncryptionAvailable() {
  // After getKey() is called once, a key is always available
